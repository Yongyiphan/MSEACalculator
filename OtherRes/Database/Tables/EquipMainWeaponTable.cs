            ...
            using (var inputStream = stream.GetInputStreamAt(0))
            {
                using (var dataReader = new DataReader(inputStream))
                {
                    ...
                    foreach (string weapItem in result.Skip(1))
                    {
                        ...
                        equip.BaseStats.FlatDEF = Convert.ToInt32(temp[8]);
                        equip.BaseStats.FlatATK = Convert.ToInt32(temp[9]);
                        equip.BaseStats.FlatMATK = Convert.ToInt32(temp[10]);
                ...
                using (SqliteCommand selectCMD = new SqliteCommand(selectQuery, dbCon))
                {
                    using (SqliteDataReader reader = selectCMD.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            equipModel.BaseStats.FlatDEF = reader.GetInt32(7);
                            equipModel.BaseStats.FlatATK = reader.GetInt32(8);
                            equipModel.BaseStats.FlatMATK = reader.GetInt32(9);
